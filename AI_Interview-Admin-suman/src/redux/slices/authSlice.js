...
const authSlice = createSlice({
	...
	reducers: {
		fetchDetailsFromStorage: (state) => {
			const token = ls.get("AI_token");
			const userData = ls.get("AI_userData");
		...
		setActiveUser: (state, { payload }) => {
			...
			ls.set("AI_token", payload.token);
			ls.set("AI_userData", payload.userData);
		},
		updateUser: (state, { payload }) => {
			state.userData = payload;
			ls.set("AI_userData", payload);
		},
