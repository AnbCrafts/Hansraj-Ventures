...
const Resume = () => {
    ...
    const [noInterview, setNoInteview] = useState([]);
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then(({ data }) => {
                ...
                setCancelled(data.data.segregated.cancelled);
                setCompleted(data.data.segregated.completed);
                setNoInteview(data.data.segregated.no_interview_doc);
                setScheduled(data.data.segregated.scheduled);
                if (active == "") setUsersData(data.data.segregated.no_interview_doc);
                if (active == "completed") setUsersData(data.data.segregated.completed);
                if (active == "scheduled") setUsersData(data.data.segregated.scheduled);
                if (active == "cancelled") setUsersData(data.data.segregated.cancelled);
                setLoading(false);
            })
            .catch(({ response }) => {
    ...
    useEffect(() => {
        if (active == "") setUsersData(noInterview);
        if (active == "completed") setUsersData(completed);
        if (active == "scheduled") setUsersData(scheDuled);
        if (active == "cancelled") setUsersData(cancelled);
    }, [active]);
