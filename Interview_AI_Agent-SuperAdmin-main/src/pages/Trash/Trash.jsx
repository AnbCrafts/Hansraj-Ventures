...
const Trash = () => {
    ...
    useEffect(() => {
        ...
        setFetchedData([]);
        axios
            .get(`/superAdmin/deleted-admins?page=${currentPage}&limit=100`)
            .then(({ data }) => {
                if (data.data.admins.length) setFetchedData(data?.data.admins);
            })
            .catch(({ response }) => {
                ...
                {loading ? (
                    ...
                    <div className={styles.UserCards}>
                        {fetchedData.length === 0 ? (
                            <h1>{!active ? "No Deleted Admins" : "No Interviews"}</h1>
                        ) : (
                            fetchedData?.map((data, index) => {
                                return (
                                    ...
                                    >
                                        ...
                                        <div>{data?.username ? data?.username : "No Username"}</div>
                                        ...
                                        <div>{data?.role ? data?.role : "No Role"}</div>
